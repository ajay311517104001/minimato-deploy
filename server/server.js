...
app.use(cors({
    credentials: true,
    origin: ['http://localhost:3000','http://localhost:8000',' http://minimato.ml/','http://18.217.159.121/']
}))
...
// app.post('/business/uploadFoodImage', passport.authenticate('jwt-canteen-signin', {session:false}), upload.single('picture'), async(req, res) => {
//     const { canteen_id } = req.user;
//     const url = `/uploads/${req.file.originalname}`
//     await pool.query('UPDATE canteen SET imageurl= $1 WHERE canteen_id = $2', [url, canteen_id])
// });
...
app.post('/api/student/signup', (req, res) => {registerClient.handleSignup(req, res, pool, bcrypt)});
app.post('/api/student/login', (req, res) => {loginClient.handleLogin(req, res, pool, bcrypt)});
app.post('/api/student/uploadImage', passport.authenticate('jwt-client-signin', {session:false}), upload.single('picture'), (req, res) => {clientUploadImage.handleUploadImage(req, res, pool)})
app.post('/api/student/order', passport.authenticate('jwt-client-signin', {session:false}), (req, res) => {clientOrder.handleClientOrder(req, res, pool)})

/*--------------------------------GET ROUTES---------------------------------------------------------------------- */

app.get('/api/student/dashboard', passport.authenticate('jwt-client-signin', {session:false}), (req, res) => {clientDashboard.handleClientDashboard(req, res)});
app.get('/api/student/getImage', passport.authenticate('jwt-client-signin', {session:false}), async (req, res) => {
...
app.get('/api/student/getCanteenDetails', async(req, res) => {
    try {        
...
app.get('/api/student/searchresults', (req, res) => {clientGetSearchResults.handleGetSearchResults(req, res, pool)})

app.get('/api/student/menu', (req,res) => {clientGetMenu.handleGetMenu(req,res,pool)})



/*--------------------------------PUT ROUTES---------------------------------------------------------------------- */

app.put('/api/student/editprofile', passport.authenticate('jwt-client-signin', {session:false}), (req, res) => {clientEditProfile.handleClientEditProfile(req, res, pool)});
